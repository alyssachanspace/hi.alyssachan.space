...
| [屬性](https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors) | * `a[target="_blank"] { ... }` 選擇所有 `<a target="_blank">` 的元素<br> * `a[href^="#"] { ... }` 選擇所有 `<a href="#...">` （開頭）的元素<br> * `[lang\|="en"] { ... }` 選擇所有 `lang="en..."` （屬性值開頭）的元素<br> * `a[href$=".pdf"] { ... }` 選擇所有 `<a href="...pdf">` （結尾）的元素<br> * `a[title~="word"] { ... }` 選擇所有 `<a title="... word ...">` （含有完整字眼）的元素<br> * `a[title*="substring"] { ... }` 選擇所有 `<a title="...substring...">` （含有字眼）的元素 |
| [反選](https://developer.mozilla.org/en-US/docs/Web/CSS/:not) | `:not(p) { ... }` 選擇所有不是 `<p>` 的元素 |
| 連結 | * [`:link { ... }`](https://developer.mozilla.org/en-US/docs/Web/CSS/:link) 選擇所有未探訪過的連結<br> * [`:visited { ... }`](https://developer.mozilla.org/en-US/docs/Web/CSS/:visited) 選擇所有已探訪過的連結<br> * [`:hover { ... }`](https://developer.mozilla.org/en-US/docs/Web/CSS/:hover) 鼠標停留時的顯示<br> * [`:active { ... }`](https://developer.mozilla.org/en-US/docs/Web/CSS/:active) 正在點選的連結 |
| 次序 | * `:first-child { ... }` 選擇第一個元素<br> * `:last-child { ... }` 選擇最後一個元素<br> * `:nth-child(even) { ... }` 選擇排在雙數的元素，即第2, 4, 6, ... 個<br> * `:nth-child(odd) { ... }` 選擇排在單數的元素，即第1, 3, 5, ... 個<br> * `:nth-child(3n+1) { ... }` 選擇排在 `3n+1` 的元素，即第3, 4, 7, ... 個 |

### <a name="css_property"></a>常用 CSS 屬性
| CSS 屬性 | 說明 | 值 |
| --- | --- | --- |
| [`background`](https://developer.mozilla.org/en-US/docs/Web/CSS/background) | 背景 | 屬性有<br> * `background-color` 背景色<br>* `background-position` 背景位置<br> * `background-repeat` 背景重複<br> * `background-size` 背景尺寸：`cover` 完全覆蓋（可能被裁）、 `contain` 全幅（可能有空白）、長度或百分比<br> * `background-image` 背景圖<br> * [`background-origin`](https://developer.mozilla.org/en-US/docs/Web/CSS/background-origin) 背景圖相對區域<br> * [`background-attachment`](https://developer.mozilla.org/en-US/docs/Web/CSS/background-attachment) 背景固定或滾動<br> *  [`background-clip`](https://developer.mozilla.org/en-US/docs/Web/CSS/background-clip) 背景延伸<br>例<br>`center / contain no-repeat url("圖片網址")`：置中 全幅 不重複 背景圖 |
...
| [`margin`](https://developer.mozilla.org/en-US/docs/Web/CSS/margin) | 外邊距 | 是 `margin-top margin-right margin-bottom margin-left` 的簡寫，以長度為值，例<br> * `10px`：四邊`10px`<br> * `10px auto`：上下`10px`，左右自動（置中）<br> * `10px auto 20px`：上邊`10px`，左右自動，下邊`20px`<br> * `10px 20px 0 30px`：上邊`10px`，右邊`20px`，下邊`0`，左邊`30px` |
...
| [`border`](https://developer.mozilla.org/en-US/docs/Web/CSS/border) |  邊框 | <p>是 `border-top border-right border-bottom border-left` 的簡寫，屬性有 `border-width border-style border-color`，例<br>`border-bottom: 1px dotted #ff0000;` <span style="border-bottom: 3px solid #ff0000;">紅色3px底線</span></p><p>`border-style`：</p><p style="line-height: 40px"><span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:none">`none`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:hidden">`hidden`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px dotted #ff0000">`dotted`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px dashed #ff0000">`dashed`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px solid #ff0000">`solid`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px double #ff0000">`double`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px groove #ff0000">`groove`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px ridge #ff0000">`ridge`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px inset #ff0000">`inset`</span> <span style="inline-block; background: #f2f2f2; padding: 5px; margin: 20px 10px 20px 0; border:3px outset #ff0000">`outset`</span></p> |
| [`border-radius`](https://developer.mozilla.org/en-US/docs/Web/CSS/border-radius) | 邊框圖角 | <p><span style="inline-block; background: #f2f2f2; padding: 10px; margin: 20px 10px 20px 0; border-radius:3px">`3px`</span><span style="inline-block; background: #f2f2f2; padding: 10px; margin: 20px 10px 20px 0; border-radius:10px">`10px`</span><span style="inline-block; background: #f2f2f2; padding: 10px; margin: 20px 10px 20px 0; border-radius:50%">`50%`</span></p> |
| [`box-sizing`](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) | 區塊尺寸 | `content-box` 預設值，以內容範圍計算總寬度及高度<br>`border-box` 以邊框範圍計算總寬度及高度<div style="position:relative;border: 1px solid;"><span style="position:absolute;top:2px;left:5px;">`margin: 40px`</span><div style="margin:40px;border:35px solid #f2f2f2;padding:30px"><span style="position:absolute;top:40px;left:45px;">`border: 35px`</span> <span style="position:absolute;top:40px;right:45px;">**`border-box`**</span><span style="position:absolute;top:75px;left:80px;">`padding: 30px`</span><div style="border: 1px dashed; padding:2px 5px; display:flex;justify-content:space-between"><p>內容範圍</p><p>**`content-box`**</p></div></div></div> |
| [`position`](https://developer.mozilla.org/en-US/docs/Web/CSS/position) | 定位 | <p>* `static` 正常佈局<br>* `relative` 相對定位<br> * `absolute` 絕對定位，以上一個相對定位為原點<br> * `fixed` 固定定位，以視窗為原點<br> * `sticky` 粘性定位</p><div style="position:relative;background:#f2f2f2;border:1px solid;padding: 5px 10px;width:200px;height:100px;margin-bottom: 50px">`position: relative;`<div style="position:absolute;top:30px;left:30px;background:#fff;border:1px solid;padding: 5px 10px;width:200px;height:100px;">`position: absolute;`<br>`top: 30px;`<br>`left: 30px;`</div></div> |
| [`object-fit`](https://developer.mozilla.org/en-US/docs/Web/CSS/object-fit) | 指定可替換元素在元素框內的對齊方式 | <p> * `fill` 預設值，完全填充<br>* `contain` 全幅<br>* `cover` 完全覆蓋<br> * `none` 保持原尺寸<br> * `scale-down` 取 `none` 或 `contain` 之間更小的一個</p> |
| [`opacity`](https://developer.mozilla.org/en-US/docs/Web/CSS/opacity) | 不透明度 | `0` 到 `1` 之間的數字，預設 `1.0`，`0` 是完全透明 |
| [`overflow`](https://developer.mozilla.org/en-US/docs/Web/CSS/overflow) | 溢出顯示 | * `visible` 預設值，內容不會被修剪，可以呈現在元素框之外<br> * `hidden` 內容將被剪裁以適合填充框，無滾動條<br> * `scroll` 內容將被剪裁以適合填充框，顯示滾動條<br> * `auto` 取決於瀏覽器，如內容溢出，桌面瀏覽器會提供滾動條<br> |
